...
import { 
  ...
  Calendar, Briefcase, Award, CheckCircle2, 
  LayoutGrid, List, FileText, ExternalLink, History, TableProperties,
  Link as LinkIcon, Info
} from 'lucide-react';
import { Modal } from '../../components/common/Modal';
import { formatCurrency, formatVnd } from '../../utils/currency';
import { calcYearsService } from '../../utils/payrollEngine';
import { PositionsModule } from '../positions/PositionsModule';

export function StaffModule({ data, activeSubTab = 'list', onSaveStaff, onSaveBhxh }) {
  ...
  // Sub-tab nội bộ (nếu người dùng bấm chuyển tab trên giao diện)
  const [currentTab, setCurrentTab] = useState(activeSubTab || 'list');

  // Cập nhật tab khi prop thay đổi
  React.useEffect(() => {
  ...
  const [activeEditTab, setActiveEditTab] = useState('personal'); // 'personal' | 'salary' | 'docs' | 'insurance'
  ...
  const handleOpenEdit = (staff) => {
    ...
    if (staff) {
      setFormData({
        ...staff,
        bac: Number(staff.bac) || 1,
        heSoLuong: Number(staff.heSoLuong) || 2.5,
        phuCapTN: Number(staff.phuCapTN) || 0,
        mucDongBhxh: Number(staff.mucDongBhxh) || Math.round((Number(staff.heSoLuong) || 2.5) * baseSalary),
        soNPT: Number(staff.soNPT) || 0,
        tenNganHang: staff.tenNganHang || 'Agribank Quý Lộc',
        ngayDamNhiemCV: staff.ngayDamNhiemCV || staff.ngayVaoLam || '01/01/2021',
        thamNienQuyDoi: Number(staff.thamNienQuyDoi) || 0,
        namVuotKhung: Number(staff.namVuotKhung) || 0,
        soQD: staff.soQD || 'NQ-01/2027/NQ-HĐQT',
        ngayQD: staff.ngayQD || '01/01/2027',
        linkHDLD: staff.linkHDLD || '',
        linkPhuLuc: staff.linkPhuLuc || '',
        linkQD: staff.linkQD || ''
      });
    } else {
  ...
  // Tự động cập nhật hệ số khi chọn chức danh hoặc bậc
  const handlePositionOrGradeChange = (chucDanhValue, bacValue) => {
    const targetChucDanh = chucDanhValue || formData.chucDanh;
    const targetBac = bacValue !== undefined ? Number(bacValue) : (Number(formData.bac) || 1);
    const matchedPos = positions.find(p => p.tenChucDanh === targetChucDanh || p.maViTri === targetChucDanh);

    if (matchedPos) {
      const heSoKey = `heSoBac${targetBac}`;
      const heSoBac = matchedPos[heSoKey] !== undefined ? Number(matchedPos[heSoKey]) : (
        targetBac === 1 ? Number(matchedPos.pa1HeSo || 2.5) :
        targetBac === 2 ? Number(matchedPos.pa2HeSo || 2.7) :
        targetBac === 3 ? Number(matchedPos.pa3HeSo || 2.9) :
        targetBac === 4 ? Number(matchedPos.pa3HeSo ? matchedPos.pa3HeSo + 0.2 : 3.1) :
        Number(matchedPos.pa3HeSo ? matchedPos.pa3HeSo + 0.4 : 3.3)
      );
      ...
      setFormData(prev => ({
        ...
        bac: targetBac,
        heSoLuong: heSoBac,
        phuCapTN: Number(matchedPos.phuCapTN) || 0,
        mucDongBhxh: Math.round(heSoBac * baseSalary)
      }));
    } else {
      setFormData(prev => ({ ...prev, chucDanh: targetChucDanh, bac: targetBac }));
    }
  };
  ...
  return (
    <div className="space-y-4">
      {/* THANH ĐIỀU HƯỚNG SUB-TAB NỘI BỘ */}
      <div className="bg-white rounded-2xl border border-slate-200 p-2 shadow-sm flex items-center justify-between">
        <div className="flex space-x-1">
          ...
          >
            ...
            <span>Danh Sách CBNV 360° ({staffList.length})</span>
          </button>
          <button
          ...
          >
            ...
            <span>Chức Danh & 5 Bậc Lương</span>
          </button>
          <button
          ...
          >
            ...
            <span>Lịch Sử Công Tác & Quyết Định</span>
          </button>
        </div>
        ...
        {currentTab === 'list' && (
          <button
            ...
            className="px-3.5 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold transition-colors flex items-center space-x-1.5 shadow-sm mr-1"
          >
      ...
      {/* ===================== TAB 2: CHỨC DANH & 5 BẬC LƯƠNG ===================== */}
      {currentTab === 'positions' && (
        <PositionsModule data={data} onSavePositions={data?.onSavePositions} />
      )}

      {/* ===================== TAB 3: LỊCH SỬ CÔNG TÁC & QUYẾT ĐỊNH ===================== */}
      {currentTab === 'history' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b pb-3">
            <div>
              <h3 className="font-bold text-slate-900 text-sm flex items-center space-x-2">
                ...
                <span>Sổ Theo Dõi Quyết Định Bổ Nhiệm, Nâng Bậc & Điều Chuyển Công Tác</span>
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Lưu trữ toàn bộ số Quyết định HĐQT, ngày hiệu lực và liên kết số hóa (Drive/PDF)
              </p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b text-[11px] font-bold text-slate-600 uppercase">
                  <th className="p-3 text-center w-12">STT</th>
                  <th className="p-3 w-16 text-center">Mã NV</th>
                  <th className="p-3 min-w-[150px]">Họ và Tên</th>
                  <th className="p-3 min-w-[140px]">Vị Trí Chức Danh</th>
                  <th className="p-3 text-center w-20">Bậc Lương</th>
                  <th className="p-3 text-right w-24">Hệ Số L1</th>
                  <th className="p-3 min-w-[140px]">Số Quyết Định</th>
                  <th className="p-3 text-center w-28">Ngày QĐ</th>
                  <th className="p-3 text-center min-w-[160px]">Tệp Văn Bản Số Hóa</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-medium">
                {staffList.map((s, idx) => (
                  <tr key={s.maNV} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3 text-center text-slate-400 font-numeric">{idx + 1}</td>
                    <td className="p-3 text-center font-numeric font-bold text-brand-navy">{s.maNV}</td>
                    <td className="p-3 font-bold text-slate-900">{s.hoTen}</td>
                    <td className="p-3 text-slate-700">{s.chucDanh}</td>
                    <td className="p-3 text-center font-numeric font-bold text-brand-navy">Bậc {s.bac || 1}</td>
                    <td className="p-3 text-right font-numeric font-bold text-blue-900">{Number(s.heSoLuong || 0).toFixed(2)}</td>
                    <td className="p-3 font-numeric font-semibold text-slate-800">{s.soQD || 'NQ-01/2027/NQ-HĐQT'}</td>
                    <td className="p-3 text-center font-numeric text-slate-600">{s.ngayQD || s.ngayVaoLam || '01/01/2027'}</td>
                      ...
                      <div className="flex items-center justify-center space-x-2">
                        {s.linkQD ? (
                          <a
                            ...
                            className="inline-flex items-center space-x-1 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-[11px] font-bold transition-colors"
                          >
                          ...
                          <span className="text-slate-400 text-[11px] italic">Chưa gắn link</span>
                        )}
                        ...
                        {s.linkHDLD && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center space-x-1 px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-[11px] font-bold transition-colors"
                          >
                            <FileText className="w-3 h-3" />
                            <span>HĐLĐ</span>
                            <ExternalLink className="w-2.5 h-2.5" />
      ...
      {currentTab === 'list' && (
        <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-slate-100 text-[11px]">
              ...
              <div className="p-2 bg-blue-50/60 rounded-lg flex items-center justify-between">
                ...
                <strong className="font-numeric text-blue-950 font-bold">
                  {(staffList.reduce((sum, s) => sum + (Number(s.heSoLuong) || 0), 0) / (staffList.length || 1)).toFixed(2)}
                </strong>
              </div>
              <div className="p-2 bg-purple-50/60 rounded-lg flex items-center justify-between">
                ...
                <strong className="font-numeric text-purple-950 font-bold">
                  {formatCurrency(staffList.reduce((sum, s) => sum + ((Number(s.heSoLuong) || 0) * baseSalary), 0))}
                </strong>
              </div>
            </div>
          </div>
          ...
          {viewMode === 'table' ? (
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 font-medium">
                    {filteredStaff.map((staff, idx) => {
                      const luongViTri = Math.round((Number(staff.heSoLuong) || 0) * baseSalary);
                      const mucBhxh = Number(staff.mucDongBhxh) || luongViTri;
                      const namDamNhiem = calcYearsService(staff.ngayDamNhiemCV || staff.ngayVaoLam);
                      const tongNamCV = namDamNhiem + (Number(staff.thamNienQuyDoi) || 0);
                      ...
                      return (
                        <tr key={staff.maNV} className="hover:bg-blue-50/30 transition-colors">
                          ...
                          <td className="p-3 text-center font-numeric font-bold text-brand-navy">
                            Bậc {staff.bac || 1}
                          </td>
                          ...
                          <td className="p-3 text-center font-numeric text-slate-700">
                            <span className="font-bold text-brand-navy">{tongNamCV} năm</span>
                            {Number(staff.namVuotKhung) > 0 && (
                              <span className="ml-1 text-[10px] bg-amber-100 text-amber-800 px-1 py-0.2 rounded">VK +{staff.namVuotKhung * 5}%</span>
                            )}
                          </td>
                          ...
                          <td className="p-3 text-center">
                            <div className="flex items-center justify-center space-x-1">
                              {staff.linkHDLD && (
                                <a href={staff.linkHDLD} target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:text-emerald-900 p-1" title="Xem HĐLĐ">
                                  <FileText className="w-3.5 h-3.5" />
                                </a>
                              )}
                              {staff.linkQD && (
                                <a href={staff.linkQD} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:text-blue-900 p-1" title="Xem Quyết định">
                                  <LinkIcon className="w-3.5 h-3.5" />
                                </a>
                              )}
                              {!staff.linkHDLD && !staff.linkQD && (
            ...
            /* HIỂN THỊ DẠNG LƯỚI THẺ (GRID VIEW) */
                  ...
                  <div key={staff.maNV} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow transition-shadow space-y-3.5">
                    ...
                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1.5 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500">Ngạch bậc & Hệ số:</span>
                        <strong className="font-numeric text-brand-navy font-bold">
                          Bậc {staff.bac || 1} • {Number(staff.heSoLuong || 0).toFixed(2)}
                        </strong>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500">Lương vị trí L1:</span>
                        <strong className="font-numeric text-slate-900 font-bold">
                          {formatCurrency(luongViTri)}
                        </strong>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500">Thâm niên chức vụ:</span>
                        <strong className="font-numeric text-slate-700">
                          {tongNamCV} năm {Number(staff.namVuotKhung) > 0 && `(VK +${staff.namVuotKhung * 5}%)`}
                        </strong>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500">Mức đóng BHXH:</span>
      ...
      {selectedStaff && (
        ...
        >
          <div className="space-y-4 text-xs">
            {/* Cụm 1: Thông tin chức danh, 5 Bậc ngạch lương & Thâm niên */}
            <div className="p-4 bg-blue-50/70 border border-blue-200 rounded-2xl space-y-3">
              <div className="flex items-center space-x-2 text-brand-navy font-bold text-sm">
                <Award className="w-4 h-4" />
                <span>CHỨC DANH CÔNG TÁC, 5 BẬC LƯƠNG & THÂM NIÊN CHỨC VỤ</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  <span className="text-slate-500">Bậc ngạch lương:</span>
                  <div className="font-numeric font-extrabold text-brand-navy text-sm mt-0.5">Bậc {selectedStaff.bac || 1} / 5</div>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-blue-200/60">
                <div>
                  <span className="text-slate-500">Thâm niên công tác (từ ngày vào làm):</span>
                  <div className="font-numeric font-bold text-slate-900 text-sm mt-0.5">
                    {calcYearsService(selectedStaff.ngayVaoLam)} năm ({selectedStaff.ngayVaoLam})
                  </div>
                </div>
                <div>
                  <span className="text-slate-500">Thâm niên chức vụ (ngày + quy đổi):</span>
                  <div className="font-numeric font-bold text-brand-navy text-sm mt-0.5">
                    {calcYearsService(selectedStaff.ngayDamNhiemCV || selectedStaff.ngayVaoLam) + (Number(selectedStaff.thamNienQuyDoi) || 0)} năm
                    {selectedStaff.thamNienQuyDoi > 0 && ` (Quy đổi: +${selectedStaff.thamNienQuyDoi}n)`}
                  </div>
                </div>
                <div>
                  <span className="text-slate-500">Vượt khung chung toàn Quỹ:</span>
                  <div className="font-numeric font-bold text-emerald-800 text-sm mt-0.5">
                    {Number(selectedStaff.namVuotKhung) > 0 ? `Bậc 5 + ${selectedStaff.namVuotKhung * 5}% (${selectedStaff.namVuotKhung} lần)` : 'Chưa vượt khung'}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-blue-200/60">
                <div>
                  <span className="text-slate-500">Lương vị trí (L1 x 2.34Tr):</span>
                  <div className="font-numeric font-bold text-slate-900 text-sm">
                    {formatCurrency(Math.round((Number(selectedStaff.heSoLuong) || 0) * baseSalary))}
                  </div>
                </div>
                <div>
                  <span className="text-slate-500">Mức đóng BHXH đăng ký:</span>
                  <div className="font-numeric font-bold text-emerald-700 text-sm">
                    {formatCurrency(Number(selectedStaff.mucDongBhxh) || Math.round((Number(selectedStaff.heSoLuong) || 0) * baseSalary))}
                  </div>
                </div>
                <div>
            ...
            {/* Cụm 2: Thông tin định danh nhân thân */}
            <div className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3">
              <div className="flex items-center space-x-2 text-slate-900 font-bold text-xs">
                <Users className="w-4 h-4 text-slate-600" />
                <span>THÔNG TIN ĐỊNH DANH & NHÂN THÂN</span>
              </div>
            ...
            {/* Cụm 3: Hợp đồng lao động & Văn bản số hóa */}
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              <div className="flex items-center space-x-2 text-slate-900 font-bold text-xs">
                <FileText className="w-4 h-4 text-slate-600" />
                <span>HỒ SƠ HỢP ĐỒNG LAO ĐỘNG & QUYẾT ĐỊNH BỔ NHIỆM</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3 bg-white border border-slate-200 rounded-xl space-y-1.5">
                  <span className="text-slate-500 block text-[11px]">Hợp đồng lao động:</span>
                  {selectedStaff.linkHDLD ? (
                    <a href={selectedStaff.linkHDLD} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1.5 text-blue-700 font-bold hover:underline">
                      <span>Mở File HĐLĐ</span>
                      <ExternalLink className="w-3 h-3" />
                    </a>
                  ) : (
                    <span className="text-slate-400 italic">Chưa gắn link Drive</span>
                  )}
                </div>

                <div className="p-3 bg-white border border-slate-200 rounded-xl space-y-1.5">
                  <span className="text-slate-500 block text-[11px]">Phụ lục HĐLĐ:</span>
                  {selectedStaff.linkPhuLuc ? (
                    <a href={selectedStaff.linkPhuLuc} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1.5 text-blue-700 font-bold hover:underline">
                      <span>Mở Phụ Lục HĐ</span>
                      <ExternalLink className="w-3 h-3" />
                    </a>
                  ) : (
                    <span className="text-slate-400 italic">Chưa gắn link Drive</span>
                ...
                <div className="p-3 bg-white border border-slate-200 rounded-xl space-y-1.5">
                  <span className="text-slate-500 block text-[11px]">Quyết định bổ nhiệm/nâng bậc:</span>
                  {selectedStaff.linkQD ? (
                    <a href={selectedStaff.linkQD} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1.5 text-blue-700 font-bold hover:underline">
                      <span>{selectedStaff.soQD || 'Mở File Quyết Định'}</span>
                      <ExternalLink className="w-3 h-3" />
                    </a>
                  ) : (
                    <span className="font-semibold text-slate-700">{selectedStaff.soQD || 'Chưa gắn link'}</span>
                  )}
                </div>
              </div>
            </div>

            {/* Cụm 4: Chính sách BHXH, Thuế & Tài khoản Ngân Hàng */}
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              <div className="flex items-center space-x-2 text-slate-900 font-bold text-xs">
      ...
      {/* ===================== MODAL FORM THÊM MỚI / CHỈNH SỬA TOÀN DIỆN ===================== */}
      {isEditing && (
        <Modal
          isOpen={true}
          ...
          <form onSubmit={handleSave} className="space-y-4 text-xs">
            ...
            <div className="flex border-b border-slate-200">
              <button
                ...
                className={`py-2.5 px-3.5 font-bold border-b-2 transition-all flex items-center space-x-1.5 ${
              ...
              >
                ...
                <span>2. Vị Trí, 5 Bậc & Thâm Niên</span>
              </button>
              <button
                type="button"
              ...
              >
                <FileText className="w-3.5 h-3.5" />
                <span>3. HĐLĐ & Quyết Định Số Hóa</span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span>4. BHXH, Thuế & Ngân Hàng</span>
              </button>
            </div>
            ...
            {/* TAB 2: VỊ TRÍ, 5 BẬC LƯƠNG & THÂM NIÊN CHỨC VỤ */}
            {activeEditTab === 'salary' && (
              <div className="space-y-3 pt-1">
                <div className="p-3 bg-blue-50/70 border border-blue-200 rounded-xl text-xs text-blue-950 flex items-start space-x-2">
                  ...
                  <div>
                    <strong>Quy chế 5 bậc ngạch vị trí:</strong> Chọn chức danh và Bậc lương (1-5), hệ thống sẽ tự động điền Hệ số lương chuẩn. Thâm niên chức vụ được tính từ ngày bổ nhiệm + số năm quy đổi tương đương.
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handlePositionOrGradeChange(e.target.value, formData.bac)}
                      className="w-full p-2 border border-slate-300 rounded-lg font-bold text-slate-900 focus:ring-1 focus:ring-brand-navy"
                    >
                      {positions.length > 0 ? (
                        positions.map(p => (
                          <option key={p.maViTri} value={p.tenChucDanh}>
                            {p.tenChucDanh} ({p.khoi})
                          </option>
                        ))
                      ) : (
                        <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Bậc Lương (1 - 5) *</label>
                    <select
                      value={formData.bac || 1}
                      onChange={(e) => handlePositionOrGradeChange(formData.chucDanh, Number(e.target.value))}
                      className="w-full p-2 border border-slate-300 rounded-lg font-numeric font-bold text-brand-navy focus:ring-1 focus:ring-brand-navy"
                    >
                      <option value="1">Bậc 1 (Năm 1-3)</option>
                      <option value="2">Bậc 2 (Năm 4-6)</option>
                      <option value="3">Bậc 3 (Năm 7-9)</option>
                      <option value="4">Bậc 4 (Năm 10-12)</option>
                      <option value="5">Bậc 5 (Trần)</option>
                    </select>
                  </div>
                  ...
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Năm Vượt Khung (Sau bậc 5)</label>
                    <input
                      type="number"
                      min="0"
                      max="8"
                      value={formData.namVuotKhung || 0}
                      onChange={(e) => setFormData({ ...formData, namVuotKhung: Number(e.target.value) || 0 })}
                      placeholder="Số lần x 5%"
                      className="w-full p-2 border border-slate-300 rounded-lg font-numeric font-bold text-purple-900 focus:ring-1 focus:ring-brand-navy"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg font-numeric font-bold text-slate-900">
                      {formatCurrency(Math.round((Number(formData.heSoLuong) || 0) * baseSalary))}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Ngày Vào Làm (Thâm niên CT)</label>
                    <input
                      type="text"
                      placeholder="dd/MM/yyyy"
                  ...
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Ngày Đảm Nhiệm Chức Vụ</label>
                    <input
                      type="text"
                      placeholder="dd/MM/yyyy"
                      value={formData.ngayDamNhiemCV || ''}
                      onChange={(e) => setFormData({ ...formData, ngayDamNhiemCV: e.target.value })}
                      className="w-full p-2 border border-slate-300 rounded-lg font-numeric font-bold text-brand-navy focus:ring-1 focus:ring-brand-navy"
                    />
                  </div>

                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Thâm Niên CV Quy Đổi (Số năm)</label>
                    <input
                      type="number"
                      min="0"
                      max="30"
                      value={formData.thamNienQuyDoi || 0}
                      onChange={(e) => setFormData({ ...formData, thamNienQuyDoi: Number(e.target.value) || 0 })}
                      className="w-full p-2 border border-slate-300 rounded-lg font-numeric font-bold focus:ring-1 focus:ring-brand-navy"
                    />
                  </div>
                </div>
            ...
            {/* TAB 3: HỢP ĐỒNG & HỒ SƠ PHÁP LÝ SỐ HÓA */}
            {activeEditTab === 'docs' && (
              <div className="space-y-3 pt-1">
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700 leading-relaxed">
                  📁 <strong>Lưu trữ liên kết tài liệu số hóa:</strong> Dán đường dẫn file PDF (Google Drive / Cloud) để ban quản trị và cán bộ có thể mở xem trực tiếp khi kiểm tra hồ sơ.
                </div>

                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Đường Dẫn File Hợp Đồng Lao Động (Link Google Drive / PDF)</label>
                  <div className="flex space-x-2">
                    <input
                      type="url"
                      placeholder="https://drive.google.com/file/d/..."
                      value={formData.linkHDLD || ''}
                      onChange={(e) => setFormData({ ...formData, linkHDLD: e.target.value })}
                      className="flex-1 p-2 border border-slate-300 rounded-lg focus:ring-1 focus:ring-brand-navy"
                    />
                    {formData.linkHDLD && (
                      <a href={formData.linkHDLD} target="_blank" rel="noopener noreferrer" className="px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 font-bold flex items-center space-x-1">
                ...
                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Đường Dẫn File Phụ Lục Hợp Đồng Lao Động</label>
                  <div className="flex space-x-2">
                    <input
                      type="url"
                      placeholder="https://drive.google.com/file/d/..."
                      value={formData.linkPhuLuc || ''}
                      onChange={(e) => setFormData({ ...formData, linkPhuLuc: e.target.value })}
                      className="flex-1 p-2 border border-slate-300 rounded-lg focus:ring-1 focus:ring-brand-navy"
                    />
                    {formData.linkPhuLuc && (
                      <a href={formData.linkPhuLuc} target="_blank" rel="noopener noreferrer" className="px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 font-bold flex items-center space-x-1">
                        <ExternalLink className="w-3.5 h-3.5" />
                        <span>Mở</span>
                      </a>
                    )}
                ...
                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Đường Dẫn File Quyết Định Bổ Nhiệm / Nâng Bậc Lương</label>
                  <div className="flex space-x-2">
                    <input
                      type="url"
                      placeholder="https://drive.google.com/file/d/..."
                      value={formData.linkQD || ''}
                      onChange={(e) => setFormData({ ...formData, linkQD: e.target.value })}
                      className="flex-1 p-2 border border-slate-300 rounded-lg focus:ring-1 focus:ring-brand-navy"
                    />
                    {formData.linkQD && (
                      <a href={formData.linkQD} target="_blank" rel="noopener noreferrer" className="px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 font-bold flex items-center space-x-1">
                        <ExternalLink className="w-3.5 h-3.5" />
                        <span>Mở</span>
                      </a>
                    )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Số Quyết Định Bổ Nhiệm / Điều Chuyển</label>
                    <input
                      type="text"
                      value={formData.soQD || ''}
                      onChange={(e) => setFormData({ ...formData, soQD: e.target.value })}
                  ...
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold">Ngày Ký Quyết Định</label>
                    <input
                      type="text"
                      placeholder="dd/MM/yyyy"
            ...
            {/* TAB 4: BHXH, THUẾ & NGÂN HÀNG */}
            {activeEditTab === 'insurance' && (
              <div className="space-y-3 pt-1">
                <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl text-xs text-emerald-950">
                  <div className="font-bold mb-0.5">Chính sách đóng BHXH linh hoạt cá nhân hóa:</div>
                  Cán bộ có thể chọn đóng đủ theo Lương vị trí (L1) hoặc chọn đóng ở mức sàn 5.000.000 ₫ hoặc mức cá nhân tự chọn. Tiền thừa 23.5% Quỹ tiết kiệm được sẽ hoàn trả lại cho cán bộ vào thu nhập.
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex space-x-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() => setFormData({ ...formData, mucDongBhxh: Math.round((Number(formData.heSoLuong) || 2.5) * baseSalary) })}
